...
def plot_majority_votes_crosstab(majority_votes):
    ...
    plt.title("Ground Truth And Majority Votes")
...
def main():
    ...
    word_count = get_most_common_words(post_df)
    ...
    print(f"Share Of Real And Fake Posts: {(real_posts_count/shape[0])*100:.2f}%, {(fake_posts_count/shape[0])*100:.2f}%")
    print("Unique Headlines Count: ", len(unique_headlines))
    print(f"Average Post Length: {average_post_len:.2f}")
    print("Most Common Words: ", word_count)
